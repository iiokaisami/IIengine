...
namespace IIEngine
{
	...
	void RGBShiftPass::Initialize(DirectXCommon* dxCommon, SrvManager* srvManager, const std::wstring& vsPath, const std::wstring& psPath)
	{
	...
	void RGBShiftPass::Draw(ID3D12GraphicsCommandList* cmdList, D3D12_GPU_DESCRIPTOR_HANDLE inputSrvHandle, ID3D12Resource* inputResource, D3D12_RESOURCE_STATES& currentState)
	{
        currentState;
        inputResource;
	...
	void RGBShiftPass::UpdateConstantBuffer()
	{
        if (mappedCB_)
        {
            *mappedCB_ = cbData_;
